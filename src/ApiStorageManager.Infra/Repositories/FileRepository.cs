...
namespace ApiStorageManager.Infra.Repositories
{
    public class FileRepository : IFileRepository
    {
        ...
        public async Task<File> GetById(Guid id, string empresa, string codigoEvento)
        {
            ...
            request.Headers.Add("Authorization", $"Bearer {_configuration["StorageInfo:AnonKey"]}");
                ...
                if(response.IsSuccessStatusCode)
                {
                    using (var stream = await response.Content.ReadAsStreamAsync())
                    {
                        using (var streamReader = new StreamReader(stream))
                        {
                            ...
                            {
                                var file = new JsonSerializer().Deserialize<File>(jsonTextReader);
                                return file;
                            }
                        }
                    }
        ...
        {
            var request = new HttpRequestMessage(HttpMethod.Post, $"{_configuration["StorageInfo:StorageEndpoint"]}/{file.UrlAddress}");
            var mediaType = new MediaTypeWithQualityHeaderValue("application/json");  
